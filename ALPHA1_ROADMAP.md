...
The goal of alpha 1 is a single, well-scoped release that:

1. **Lets primrosehill migrate off `desktop/shared/undo/`.** That codebase is the
...
3. **Avoids feature creep.** Branching, selective undo, CRDT, reducer
   integration, and patch-based command synthesis are explicitly deferred. They
   are real wants but each carries enough design surface to derail a focused
   alpha.

`0.1.0` shipped with: synchronous `Command { redo, undo, label }`, single-store
`AmnesiaProvider`, `useAmnesia`, `useUndoableState`, `<AmnesiaShortcuts />`,
optional `usePersistedUndoableState` via `react-mnemonic`. Everything below
extends that core; no current public API is broken without a deprecation
window.
...
- `0.1.x` — current. Patch-only fixes; no new features.
- `0.2.0-alpha.1` — this roadmap. Public API additions only.
- `0.2.0-alpha.2..N` — bug-fix / API-tightening pass; cut after at least one
  external adopter has integrated.
- `0.2.0` — stable. Doc surfaces (`llms.txt` etc.) and migration guide finalized.

Breaking changes since `0.1.0` are kept narrow:

- `Command.redo` is **renamed to `Command.do`** with `Command.redo` aliased for
  one minor cycle (deprecation warning in dev). Rationale: matches primrosehill
  vocabulary and the React Hook Form / Slate convention.
- `Amnesia.push`, `Amnesia.undo`, `Amnesia.redo` return `Promise<…>` instead of
  the bare value. Synchronous call sites that ignore the return value are
  unaffected; call sites that consume the entry id must `await` (or use
  `void store.undo()`).

Both changes ship behind a `compat` re-export so consumers can opt out for one
release. Document in `MIGRATION.md`.
...
- For one minor, accept `Command.redo` as the only handler (no `do`) — current
  behavior. Emit a dev-mode console hint when `do` is absent and `applied !== true`.

**Acceptance:**

- New test: `push({ do, redo, undo })` with `applied: false` calls `do` once,
  then `redo` on each subsequent redo.
- Backward-compat test: `push({ redo, undo })` still works unchanged.
...
react-amnesia today gives one store per `AmnesiaProvider`, and the only way to
have two histories is to mount two providers, which doesn't compose cleanly
with shortcuts.
...
- New API:
  ...
  - `useAmnesiaScope(scopeId)` — explicitly enter a scope, returns the same
    store shape, used by long-lived authoring surfaces.
  ...
  - `provider.activeScopeId` — read-only string in the snapshot.
...
doc (Recipe 8 — multi-key persisted action) all hand-roll the "wrap N
mutations in one entry" pattern by capturing previous values manually and
composing closures. A first-class transaction API makes this safe.
...
`event.altKey === false`. We don't. Also: today we install on `window` by
default, which is fine, but we should document and test the
...
- Update peer dep range to `react: >=18 <20`.
...
- Each `do` / `undo` / `redo` invocation receives an optional second argument:
  `(signal: AbortSignal) => void | Promise<void>`.
- `clear()`, scope teardown, and provider unmount call `abort()` on the active
  signal.
- A rejected promise whose rejection is the signal's `AbortError` is treated
  as a successful no-op rather than an `onError` event.
...
- Test: a long-running `await push({ do: async (_, signal) => …, undo })`
  followed by `clear()` aborts the in-flight `do` and does not push the entry.
...
These are not features but ship-blockers for the alpha.

- **`MIGRATION.md`** with the `Command.redo` → `Command.do` deprecation, the
  async-return-type change, and the multi-scope opt-in story.
- **`CHANGELOG.md`** with `0.2.0-alpha.1` entry following Keep-a-Changelog.
...
- At least one external adopter integration before promoting to non-alpha.
  Ideally primrosehill itself.

---

## Explicitly deferred to alpha 2 or later
...
2. `npm run lint`, `npm run test`, `npm run ai:check`, `npm run build` all pass
   in CI on every PR.
3. `MIGRATION.md` exists and one external adopter (primrosehill or a fixture
   app) has integrated against the alpha branch end-to-end.
4. Canonical AI docs are regenerated; `AGENTS.md` / `CLAUDE.md` / Cursor /
   Copilot rules reflect the new API.
5. Bundle-size budgets pass.
6. Tag `v0.2.0-alpha.1`, publish to npm under the `alpha` dist-tag, announce.
...
(after all features) ─► cross-cutting (docs, migration, budgets, adoption)
```
...
- Do not gate adoption on the website / `llms.txt` / typedoc work in
  `DOCS_TODO.md`. Those are valuable but orthogonal — alpha 1 should ship the
  runtime even if the docs site lags.
